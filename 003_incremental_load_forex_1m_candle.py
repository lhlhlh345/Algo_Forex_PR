...
msg = f"The ETL job for refreshing forex_precious_metal_1m_jason_stage table is completed on {dt.date.today().strftime('%Y/%m/%d')}. Sending at {dt.datetime.now().strftime('%H:%M:%S.%f')}, and elapsed time is {Elapsed_Time}. Update Message: {body}"
